...
- Name: Sign up and save a storage state of the first user
- Location: D:\QA Madness\gitea-practice\tests\setup\auth.spec.ts:8:5
...
Call log:
  - navigating to "http://localhost:3000/user/sign_up", waiting until "load"
    ...
    at D:\QA Madness\gitea-practice\tests\setup\auth.spec.ts:13:24
```

# Page snapshot

```yaml
- heading "Немає зв’язку із сайтом" [level=1]
- paragraph:
  - text: Хост
  - strong: localhost
  - text: відхилив запит на з’єднання.
- paragraph: "Спробуйте:"
- list:
  - listitem: перевірити наявність з’єднання
  - listitem:
    - link "перевірити проксі-сервер і брандмауер":
      - /url: "#buttons"
- text: ERR_CONNECTION_REFUSED
- button "Перезавантажити"
- button "Деталі"
```
